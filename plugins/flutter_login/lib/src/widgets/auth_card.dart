          ...
          final child = (index == 0)
              ? _buildLoadingAnimator(
                  ...
                  child: _LoginCard(
                    ...
                    onSubmitCompleted: (loginCompleted) {
                      if (loginCompleted) {
                        _forwardChangeRouteAnimation().then((_) {
                          widget?.onSubmitCompleted(loginCompleted);
                        });
                      } else {
                        widget?.onSubmitCompleted(loginCompleted);
                      }
...
class _LoginCardState extends State<_LoginCard> with TickerProviderStateMixin {
  ...
  Future<bool> _submit() async {
    ...
    final loginCompleted = auth.isLogin ? true : false;  // sign up by email, after sign up, still need to activate in email.
